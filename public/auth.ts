type Role = 'user' | 'mentor' | 'therapist';
...
function postAuthUrl(user: CurrentUser): string {
  if (!user.role) return 'choose-profile.html';
...
function updateSigninLinks(user: CurrentUser | null): void {
  const links = document.querySelectorAll<HTMLAnchorElement>('.nav-signin');
  links.forEach((link) => {
    if (user) {
...
function redirectIfSignedIn(user: CurrentUser | null): void {
  if (user && window.location.pathname.endsWith('signin.html')) {
    window.location.href = postAuthUrl(user);
  }
}

function enforceDashboardAccess(user: CurrentUser | null): boolean {
  const requiredRole = document.body.dataset.requiredRole as Role | undefined;
  const requiresAdmin = document.body.dataset.requiresAdmin === 'true';
  if (!requiredRole && !requiresAdmin) return true;
...
function enforceProfileAccess(user: CurrentUser | null): boolean {
...
function enforceChooseProfileAccess(user: CurrentUser | null): boolean {
  const requiresChoice = document.body.dataset.chooseProfile === 'true';
  if (!requiresChoice) return true;
...
function renderDashboardUser(user: CurrentUser | null): void {
  if (!user) return;
  const nameEls = document.querySelectorAll<HTMLElement>('.user-name-slot');
  nameEls.forEach((el) => {
...
function showFormMessage(el: HTMLElement | null, message: string, isError: boolean): void {
  if (!el) return;
...
function setupAuthTabs(): void {
  const tabButtons = document.querySelectorAll<HTMLButtonElement>('.auth-tab');
...
function setupSelectRoleForm(): void {
  const form = document.getElementById('role-select-form') as HTMLFormElement | null;
  if (!form) return;
  ...
  function syncTherapistFields(): void {
    if (!therapistFields) return;
    const selected = form!.querySelector<HTMLInputElement>('input[name="role"]:checked');
  ...
  form.addEventListener('submit', async (event) => {
    event.preventDefault();
    const roleInput = form.querySelector<HTMLInputElement>('input[name="role"]:checked');
    if (!roleInput) {
      showFormMessage(messageEl, 'Please choose a profile type.', true);
      return;
    }
    const role = roleInput.value as Role;
    const payload: { role: Role; email?: string; phone?: string; credentials?: string } = { role };

    if (role === 'therapist') {
      const emailInput = document.getElementById('therapist-email') as HTMLInputElement | null;
      const phoneInput = document.getElementById('therapist-phone') as HTMLInputElement | null;
      const credentialsInput = document.getElementById('therapist-credentials') as HTMLTextAreaElement | null;
...
function setupLogoutButtons(): void {
  const buttons = document.querySelectorAll<HTMLButtonElement>('.logout-btn');
...
function setAvatarPreview(url: string | null, fallbackInitial: string): void {
...
function renderProfilePage(user: CurrentUser | null): void {
  const form = document.getElementById('profile-form');
  if (!form || !user) return;

  const nameInput = document.getElementById('profile-display-name') as HTMLInputElement;
  const bioInput = document.getElementById('profile-bio') as HTMLTextAreaElement;
  ...
  const backLink = document.getElementById('back-to-dashboard') as HTMLAnchorElement | null;
  if (backLink) {
...
function setupAvatarUpload(): void {
  ...
  input.addEventListener('change', () => {
    ...
    reader.onload = () => {
      pendingAvatarDataUrl = reader.result as string;
...
function setupProfileForm(user: CurrentUser | null): void {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const result = await postJson('/api/update-profile', {
      ...
      bio: bioInput.value,
      avatarUrl: pendingAvatarDataUrl ?? user?.avatarUrl ?? null,
    });
    showFormMessage(messageEl, result.message, !result.success);
  });
}
...
interface ActiveTherapistEntry {
  username: string;
  created_at: string;
}

async function setupAdminPanel(): Promise<void> {
  ...
  const data = (await response.json()) as { pending: TherapistEntry[]; active: ActiveTherapistEntry[] };
