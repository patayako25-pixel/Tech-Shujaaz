"use strict";
(function () {
    const SESSION_RATES = { virtual: 2000, physical: 3000 };
    const FEE_RATE = 0.1;
    const CURRENCY = 'KES';
    let rows = [];
    let range = '30';
    ...
    const typeLabel = (t) => (t === 'physical' ? 'Physical' : 'Virtual');
    const statusLabel = (s) => (s === 'paid' ? 'Paid' : 'Pending');
    ...
    function renderChart() {
        ...
        axis.innerHTML = '';
        const paid = rows.filter((r) => r.status === 'paid');
        const buckets = buildBuckets().map((bucket) => (Object.assign(Object.assign({}, bucket), { total: sum(paid.filter((r) => r.date >= bucket.start && r.date < bucket.end), 'net') })));
    ...
    function filteredRows() {
        const status = byId('pay-filter').value;
        const term = byId('pay-search').value.trim().toLowerCase();
        return rows.filter((r) => {
            if (status !== 'all' && r.status !== status)
                return false;
            if (term && r.client.toLowerCase().indexOf(term) === -1)
                return false;
            return true;
        });
    }
    ...
    function renderTable() {
        ...
        list.forEach((r) => {
            ...
            tr.appendChild(cell(r.date.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }), 'date'));
    ...
    function exportCsv() {
        const lines = [['Date', 'Client', 'Type', 'Status', 'Charge', 'Fee', 'Net']];
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const trigger = document.querySelector('.sidebar-link[data-view="payments"]');
        if (!trigger || !document.getElementById('view-payments'))
            return;
        ...
        const rangeButtons = document.querySelectorAll('.pay-range button');
