...
router.get('/therapists', async (_req, res) => {
    const result = await db_1.default.query(`SELECT username, display_name, bio, avatar_url FROM users
...
router.get('/therapists/:username/slots', async (req, res) => {
    const result = await db_1.default.query(`SELECT id, start_time, end_time, session_type FROM therapist_slots
     WHERE therapist_username = $1 AND status = 'open' AND start_time > now()
     ORDER BY start_time ASC`, [req.params.username]);
    res.json({ success: true, slots: result.rows });
});
router.get('/my-slots', (0, auth_helpers_1.requireRole)('therapist'), async (req, res) => {
    const currentUser = req.currentUser;
    const result = await db_1.default.query(`SELECT ts.id, ts.start_time, ts.end_time, ts.session_type, ts.status, b.user_username AS booked_by
     ...
     ORDER BY ts.start_time ASC`, [currentUser.username]);
    res.json({ success: true, slots: result.rows });
});
router.post('/slots', (0, auth_helpers_1.requireRole)('therapist'), async (req, res) => {
    const currentUser = req.currentUser;
    ...
    const result = await db_1.default.query(`INSERT INTO therapist_slots (therapist_username, start_time, end_time, session_type)
     VALUES ($1, $2, $3, $4) RETURNING id, start_time, end_time, session_type, status`, [currentUser.username, startTime, endTime, sessionType]);
    res.json({ success: true, message: 'Slot added.', slot: result.rows[0] });
});
router.delete('/slots/:id', (0, auth_helpers_1.requireRole)('therapist'), async (req, res) => {
    const currentUser = req.currentUser;
    const result = await db_1.default.query(`DELETE FROM therapist_slots
     WHERE id = $1 AND therapist_username = $2 AND status = 'open'
     RETURNING id`, [req.params.id, currentUser.username]);
...
router.post('/bookings', (0, auth_helpers_1.requireRole)('user'), async (req, res) => {
    const currentUser = req.currentUser;
    ...
    try {
        ...
        const slotResult = await client.query(`SELECT id, therapist_username, start_time FROM therapist_slots
       WHERE id = $1 AND status = 'open' AND start_time > now() FOR UPDATE`, [slotId]);
        ...
        await client.query(`INSERT INTO bookings (slot_id, user_username, therapist_username)
       VALUES ($1, $2, $3)`, [slot.id, currentUser.username, slot.therapist_username]);
...
router.get('/my-bookings', (0, auth_helpers_1.requireRole)('user'), async (req, res) => {
    const currentUser = req.currentUser;
    const result = await db_1.default.query(`SELECT b.id, b.status, ts.start_time, ts.end_time,
